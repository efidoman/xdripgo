...
func findControlServiceAndControl(dev *api.Device) {
	...
	if err != nil {
		...
		g := NewGlucose(gluc_message, time_rx_message, sensor_message, sync_date, int(props.RSSI), props.Name)
...
func filterDevice(dev *api.Device, name string) bool {
	...
	if props.Name != name {
		...
		log.Debugf("found(%s) rssi(%d), wanted(%s)", props.Name, props.RSSI, name)
		return true
	}
}
